...
{
	if ( OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus) /* If device is connected */
	{
		*DeviceSpeed =  ( OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_LowSpeedDeviceAttached) ? LOW_SPEED : FULL_SPEED;
...
HCD_STATUS HcdRhPortReset(uint8_t HostID, uint8_t uPortNumber)
{
	...
	OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortResetStatus; /* SetPortReset */
	/* should have time-out */
	while ( OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_PortResetStatus) {}

	OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortResetStatusChange; /* Clear Port Reset Status Change */
...
{
	OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PowerEnableStatus; /* SetPortEnable */
...
{
	OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_CurrentConnectStatus; /* ClearPortEnable */
...
HCD_STATUS HcdCancelTransfer(uint32_t PipeHandle)
{
	...
	OHCI_REG(HostID)->InterruptStatus = HC_INTERRUPT_StartofFrame;
	while( !(OHCI_REG(HostID)->InterruptStatus & HC_INTERRUPT_StartofFrame) ) /* TODO Should have timeout */
...
HCD_STATUS HcdControlTransfer(uint32_t PipeHandle, const USB_Request_Header_t* const pDeviceRequest, uint8_t* const buffer)
{
	...
	OHCI_REG(HostID)->CommandStatus |= HC_COMMAND_STATUS_ControlListFilled;
...
HCD_STATUS HcdDataTransfer( uint32_t PipeHandle, uint8_t* const buffer, uint32_t const length, uint16_t* const pActualTransferred)
{
	...
	}else
	{
		...
		{
			OHCI_REG(HostID)->CommandStatus |= HC_COMMAND_STATUS_BulkListFilled;
		}
	}
...
void HcdIrqHandler(uint8_t HostID)
{
	...
	IntStatus = OHCI_REG(HostID)->InterruptStatus & OHCI_REG(HostID)->InterruptEnable ;

	if (IntStatus == 0)
		return;

	/* disable all interrupt for processing */
	OHCI_REG(HostID)->InterruptDisable = HC_INTERRUPT_MasterInterruptEnable;
	...
	if (IntStatus & HC_INTERRUPT_RootHubStatusChange)
	{
		...
		if (OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_ConnectStatusChange)
		{
			if (OHCI_REG(HostID)->RhStatus & HC_RH_STATUS_DeviceRemoteWakeupEnable)	/* means a remote wakeup event */
			{
			...
			OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_ConnectStatusChange; /* clear CSC bit */
			OHciRhStatusChangeIsr(HostID, OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus);
		}

		if (OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_PortEnableStatusChange)
		{
			OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortEnableStatusChange; /* clear PESC */
		}

		if (OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_PortSuspendStatusChange)
		{
			OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortSuspendStatusChange;			/* clear PSSC */
		}

		if (OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_OverCurrentIndicatorChange) /* Over-current handler to avoid physical damage */
		{
			OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_OverCurrentIndicatorChange;			/* clear OCIC */
		}

		if (OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_PortResetStatusChange)
		{
			OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortResetStatusChange;			/* clear PRSC */
		}
	}
...
#if SCHEDULING_OVRERRUN_INTERRUPT
	if (OHCI_REG(HostID)->InterruptStatus & HC_INTERRUPT_SchedulingOverrun)
	{
...
#if SOF_INTERRUPT
	if (OHCI_REG(HostID)->InterruptStatus & HC_INTERRUPT_StartofFrame)
	{
...
#if RESUME_DETECT_INTERRUPT
	if (OHCI_REG(HostID)->InterruptStatus & HC_INTERRUPT_ResumeDetected)
	{
...
#if UNRECOVERABLE_ERROR_INTERRUPT
	if (OHCI_REG(HostID)->InterruptStatus & HC_INTERRUPT_UnrecoverableError)
	{
...
#if FRAMENUMBER_OVERFLOW_INTERRUPT
	if (OHCI_REG(HostID)->InterruptStatus & HC_INTERRUPT_FrameNumberOverflow)
	{
...
#if OWNERSHIP_CHANGE_INTERRUPT
	if (OHCI_REG(HostID)->InterruptStatus & HC_INTERRUPT_OwnershipChange)
	{
		OHciOwnershipChangeIsr(HostID);
	}
#endif

	OHCI_REG(HostID)->InterruptStatus = IntStatus; /* Clear InterruptStatus */
	OHCI_REG(HostID)->InterruptEnable = HC_INTERRUPT_MasterInterruptEnable;
}
...
static __INLINE HCD_STATUS OHciHostInit(uint8_t HostID)
{
	...
	OHCI_REG(HostID)->Control = 0;
	OHciHostOperational(HostID); /* have to turn HC to operational mode before setting up below registers*/	

	OHCI_REG(HostID)->FmInterval = HC_FMINTERVAL_DEFAULT;
	OHCI_REG(HostID)->PeriodicStart = PERIODIC_START;

	OHCI_REG(HostID)->ControlHeadED = (uint32_t) &(ohci_data[HostID].staticEDs[CONTROL_LIST_HEAD]);
	OHCI_REG(HostID)->BulkHeadED = (uint32_t) &(ohci_data[HostID].staticEDs[BULK_LIST_HEAD]);

	OHCI_REG(HostID)->HCCA = (uint32_t) &(ohci_data[HostID].hcca);	/* Hook Hcca */

	/* Set up Control */
	OHCI_REG(HostID)->Control |= CONTROL_BULK_SERVICE_RATIO |
	...
	/* Set Global Power */
	OHCI_REG(HostID)->RhStatus = HC_RH_STATUS_LocalPowerStatusChange;

	// HcInterrupt Registers Init
	OHCI_REG(HostID)->InterruptStatus |= OHCI_REG(HostID)->InterruptStatus; /* Clear Interrupt Status */
	OHCI_REG(HostID)->InterruptDisable = HC_INTERRUPT_ALL; /* Disable all interrupts */
	/* Enable necessary Interrupts */
	OHCI_REG(HostID)->InterruptEnable = HC_INTERRUPT_MasterInterruptEnable | HC_INTERRUPT_WritebackDoneHead | HC_INTERRUPT_RootHubStatusChange |
...
{
	OHCI_REG(HostID)->CommandStatus = HC_COMMAND_STATUS_HostControllerReset;
	while( OHCI_REG(HostID)->CommandStatus & HC_COMMAND_STATUS_HostControllerReset){} /* FIXME Wait indefinitely (may need a time-out here) */
...
{
	OHCI_REG(HostID)->Control = (OHCI_REG(HostID)->Control & (~HC_CONTROL_HostControllerFunctionalState)) | (HC_HOST_SUSPEND << 6) ;
	return HCD_STATUS_OK;
}

static __INLINE HCD_STATUS OHciHostOperational(uint8_t HostID)
{
	OHCI_REG(HostID)->Control = (OHCI_REG(HostID)->Control & (~HC_CONTROL_HostControllerFunctionalState)) | (HC_HOST_OPERATIONAL << 6) ;
	return HCD_STATUS_OK;
}
...
{
	OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortPowerStatus; /* SetPortPower */
	HcdDelayMS( 2*( (OHCI_REG(HostID)->RhDescriptorA & HC_RH_DESCRIPTORA_PowerOnToPowerGoodTime) >> 24 ) ); /* FIXME need to delay here POTPGT */
...
{
	OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_LowSpeedDeviceAttached; /* ClearPortPower */
	return HCD_STATUS_OK;
}
...
static __INLINE HCD_STATUS OHciRhPortSuspend(uint8_t HostID, uint8_t uPortNumber)
{
	if ( OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus) /* If device is connected */
	{
		OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortSuspendStatus; /* SetPortSuspend */
	}
...
{
	if ( OHCI_REG(HostID)->RhPortStatus1 & HC_RH_PORT_STATUS_CurrentConnectStatus) /* If port is currently suspended */
	{
		OHCI_REG(HostID)->RhPortStatus1 = HC_RH_PORT_STATUS_PortOverCurrentIndicator; /* ClearSuspendStatus */
	}
